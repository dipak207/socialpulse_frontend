...
import { submitProfileAnalysis } from '@/lib/api'
...
export default function ProfileAnalysisPage() {
  ...
  const handleSubmit = async (url: string, type: 'post' | 'profile', timeframe: string) => {
    ...
    setResult(null)
    try {
      const job = await submitProfileAnalysis(url)
  ...
  return (
    <div className="min-h-screen bg-navy-900">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold gradient-text">Profile Analysis</h1>
          <p className="text-muted-foreground mt-1">Analyze a creator's overall profile performance</p>
        </div>
